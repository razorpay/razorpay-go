...
func TestCreateRegistrationLink(t *testing.T) {
	teardown, fixture := utils.StartMockServer("/subscription_registration/auth_links", "fake_subscription_registration")
	defer teardown()
	params := map[string]interface{}{
	...
	body, err := utils.Client.Subscription.CreateRegistrationLink(params, nil)
	jsonByteArray, _ := json.Marshal(body)
	assert.Equal(t, err, nil)
	utils.TestResponse(jsonByteArray, []byte(fixture), t)
}